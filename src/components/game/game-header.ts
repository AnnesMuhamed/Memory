...
import { PAWN_IMAGES, PLAYER_LABEL_IMAGES } from '../../constants/assets';
...
function getPlayerIconSrc(player: Player): string {
  if (settings.theme === 'code-vibes') {
    return PLAYER_LABEL_IMAGES[player];
  }
  return PAWN_IMAGES[player];
}
...
function applyScoreIcons(): void {
  ...
  if (settings.theme === 'code-vibes') {
    blueIcon.src = PLAYER_LABEL_IMAGES.blue;
    orangeIcon.src = PLAYER_LABEL_IMAGES.orange;
    return;
  }
  blueIcon.src = PAWN_IMAGES.blue;
  orangeIcon.src = PAWN_IMAGES.orange;
}
